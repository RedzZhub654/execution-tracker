...
-- ===== Execution Tracker =====
-- Set these two values once. URL = your worker address, SECRET = the token
-- from the tracker's /settings page. Game names are detected automatically,
-- so you do NOT type any game names here.
local HttpService = game:GetService("HttpService")
local TRACKER_URL = "https://execution-tracker.YOUR-SUBDOMAIN.workers.dev/api/report"
local TRACKER_SECRET = "YOUR-SECRET-TOKEN"

local function reportExecution(gameName)
    local q = "?game=" .. HttpService:UrlEncode(gameName) .. "&secret=" .. TRACKER_SECRET
    pcall(function()
        HttpService:PostAsync(TRACKER_URL .. q, "")
    end)
end

local function cleanName(file)
    local name = file:gsub("%.lu$", ""):gsub("%.lua$", "")
    name = name:gsub("%-", " ")
    name = name:gsub("(%a)(%w*)", function(first, rest) return first:upper() .. rest end)
    return name
end
-- ===== End Execution Tracker =====
